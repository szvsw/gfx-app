...
export const EquilibriumNode: React.FC = () => {
  const createEquilibriumNode = useCallback(() => {
    return {
      ...
      initializeAngles() {
        this.angles = Array(this.nForces)
          .fill(0)
          .map((a, i) => (2 * i * Math.PI) / (this.nForces + 1))
        this.magnitudes = Array(this.nForces).fill(100)
      },
      sortAngles() {
        const magnitudesWithKeys = this.magnitudes.map((a, i) => {
          return { id: i, value: a }
        })
        magnitudesWithKeys.sort(
          (a: { id: number }, b: { id: number }) => this.angles[a.id] - this.angles[b.id]
        )
        ...
        this.pointsInForce.map((elem, i) => {
          switch (i) {
            case 0:
              ...
              break
            default:
              this.pointsInForce[i] = [
                this.pointsInForce[i - 1][0] - this.vectors[i - 1][0] * this.magnitudes[i - 1],
                this.pointsInForce[i - 1][1] - this.vectors[i - 1][1] * this.magnitudes[i - 1],
              ]
              break
          }
        })
        this.vectors[this.nForces - 1] = [
          this.pointsInForce[this.nForces - 1][0] - this.pointsInForce[0][0],
          this.pointsInForce[this.nForces - 1][1] - this.pointsInForce[0][0],
        ]
        this.magnitudes[this.nForces - 1] = Math.sqrt(
          Math.pow(this.vectors[this.nForces - 1][0], 2) +
            Math.pow(this.vectors[this.nForces - 1][1], 2)
        )
      },
      drawForm() {
        this.p5?.push()
        ...
        this.vectors.map(([x, y], i) => {
          ...
          this.p5?.stroke(255)
          this.p5?.line(
            (vector.x / this.magnitudes[i]) * 12,
            (vector.y / this.magnitudes[i]) * 12,
            vector.x,
            vector.y
          )
          this.p5?.line(
            vector?.x - cutLineVector.x,
            vector?.y - cutLineVector.y,
            vector?.x + cutLineVector?.x,
            vector?.y + cutLineVector.y
          )
          ...
          this.p5?.line(
            (vector.x / this.magnitudes[i]) * 12,
            (vector?.y / this.magnitudes[i]) * 12,
            (vector.x / this.magnitudes[i]) * 12 + arrowHeadVector?.x,
            (vector.y / this.magnitudes[i]) * 12 + arrowHeadVector.y
          )
    ...
    <Box sx={{ mt: '1rem' }}>
      ...
      <Stack spacing="1rem" direction="row">
        <Stack sx={{ mt: '0.25rem', width: '30%' }} spacing="0.5rem">
          {equilibriumNode.current.angles.map(
            (angle, i) =>
              i < equilibriumNode.current.angles.length - 1 && (
        ...
        <Stack sx={{ mt: '0.25rem', width: '30%' }} spacing="0.5rem">
          {equilibriumNode.current.magnitudes.map(
            (mag, i) =>
              i < equilibriumNode.current.magnitudes.length - 1 && (
