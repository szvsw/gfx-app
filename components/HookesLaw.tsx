...
export default function HookesLaw() {
  const createHookesLaw = useCallback(() => {
    return {
      ...
      evaluateNearestBoundary([x, y]: number[]) {
        const d0 = this.p5
          ?.createVector(this.edgeConditionsInForm[0][0], this.edgeConditionsInForm[0][1])
          .sub(this.p5?.createVector(x, y))
          .mag()
        const d1 = this.p5
          ?.createVector(this.edgeConditionsInForm[1][0], this.edgeConditionsInForm[1][1])
          .sub(this.p5?.createVector(x, y))
          .mag()
        return d0 < d1 ? 0 : 1
      },
      equallySpaceLoadsInForm() {
        const loadSpacing =
          (this.edgeConditionsInForm[1][0] - this.edgeConditionsInForm[0][0]) / this.nSegments
        this.loadXCoordsInForm = Array(this.nSegments - 1)
          .fill(1)
          .map((a, i) => this.edgeConditionsInForm[0][0] + loadSpacing + i * loadSpacing)
      },
      ...
      updateLoadLine() {
        this.loadYCoordsInForce = Array(this.loads.length + 1)
          .fill(0)
          .map((a, i) => this.loads.reduce((a, b, j) => a + (j < i ? b : 0), 0))
      },
      updateClosingLineSlope() {
        this.closingLineSlope =
          (this.edgeConditionsInForm[0][1] - this.edgeConditionsInForm[1][1]) /
          (this.edgeConditionsInForm[0][0] - this.edgeConditionsInForm[1][0])
      },
      updateCoM() {
        ...
        const yCoord =
          this.closingLineSlope * (xCoord - this.edgeConditionsInForm[0][0]) +
          this.edgeConditionsInForm[0][1]
      ...
      updatePoleInForce() {
        ...
        const xIntersection =
          (bottomPoint - topPoint) / (this.tangentSlopes[0] - this.tangentSlopes[1])
        const yIntersection = this.tangentSlopes[0] * xIntersection + topPoint
        this.poleInForce = [xIntersection, yIntersection]
      },
      updateTangentSlopes() {
        const slopeA =
          (this.edgeConditionsInForm[0][1] - this.poleInForm[1]) /
          (this.edgeConditionsInForm[0][0] - this.poleInForm[0])
        const slopeB =
          (this.edgeConditionsInForm[1][1] - this.poleInForm[1]) /
          (this.edgeConditionsInForm[1][0] - this.poleInForm[0])
      ...
      solveFormDiagram() {
        ...
        this.nodesInForm.map((a, i) => {
          switch (i) {
            case 0:
              this.nodesInForm[i] = [
                this.edgeConditionsInForm[0][0],
                this.edgeConditionsInForm[0][1],
              ]
              break
            case this.nodesInForm.length - 1:
              this.nodesInForm[i] = [
                this.edgeConditionsInForm[1][0],
                this.edgeConditionsInForm[1][1],
              ]
              break
            default:
              const tensionForceSlope =
                (this.poleInForce[1] - this.loadYCoordsInForce[i - 1]) / this.poleInForce[0]
              const yIntersection =
                tensionForceSlope * (this.loadXCoordsInForm[i - 1] - this.nodesInForm[i - 1][0]) +
                this.nodesInForm[i - 1][1]
      ...
      drawClosingLineInForm() {
        ...
        while (start.x < (end?.x ?? -999)) {
          this.p5?.line(
            ...
            start?.y,
            start?.x + dashSize,
            start?.y + dashSize * this.closingLineSlope
          )
      ...
      drawTangentsInForm() {
        ...
        while (start.x < this.poleInForm[0]) {
          this.p5?.line(
            ...
            start?.y,
            start?.x + dashSize,
            start?.y + dashSize * this.tangentSlopes[0]
          )
        ...
        while (start.x < this.edgeConditionsInForm[1][0]) {
          this.p5?.line(
            ...
            start?.y,
            start?.x + dashSize,
            start?.y + dashSize * this.tangentSlopes[1]
          )
      ...
      drawMirrorNodesInForm() {
        ...
        this.nodesInForm.map(([x, y]) => {
          this.p5?.ellipse(
            x,
            -y,
            (formDiagramEllipseSize * 0.5) / this.formDiagramScale,
            (formDiagramEllipseSize * 0.5) / this.formDiagramScale
          )
        })
      },
  ...
  return (
    <Box sx={{ mt: '1rem' }}>
      <Typography>
        Click near one of the form edges to relocate it. Click again to return to pole-position
        editing.
      </Typography>
      ...
      <Sketch
        ...
        draw={(p5: p5Types) => {
          ...
          if ([0, 1].includes(editingEdge)) {
            hookesLaw.current.edgeConditionsInForm[editingEdge] =
              hookesLaw.current.evaluateMouseClickInFormDiagram()
        ...
        mouseClicked={(p5: p5Types) => {
          if (editingEdge == -1) {
            setEditingEdge(
              hookesLaw.current.evaluateNearestBoundary(
                hookesLaw.current.evaluateMouseClickInFormDiagram()
              )
            )
          } else {
