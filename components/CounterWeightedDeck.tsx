...
export const CalatravaSketch: React.FC = () => {
  const createCalatrava = useCallback(() => {
    return {
      ...
      updateTowerVector() {
        this.towerVector = [
          this.towerVectorMagnitude * Math.cos(this.towerVectorAngle),
          this.towerVectorMagnitude * Math.sin(this.towerVectorAngle),
        ]
      },
      updateDeck() {
        const deckOffset = this.deckVector.map((a: number) => a * 0.5)
        this.deckPointsInForm = Array(this.nSegmentsInDeck)
          .fill([0, 0])
          .map((elem, i) => {
      ...
      updateTower() {
        ...
        this.towerPointsInForm = Array(this.nSegmentsInDeck)
          .fill([0, 0])
          .map((elem, i) => {
      ...
      updateCableVectorsAndPoints() {
        ...
        this.cableVectors = Array(this.nSegmentsInDeck)
          .fill([0, 0])
          .map((elem, i) => {
            const vect = this.towerPointsInForm[i].map(
              (coord, j) => coord - this.deckPointsInForm[i][j]
            )
            const vectMag = Math.sqrt(vect[0] * vect[0] + vect[1] * vect[1])
            const unitVect = vect.map((coord) => coord / vectMag)
            this.cablePointsInForce[i + 1] = this.cablePointsInForce[i].map(
              (coord, j) => coord + unitVect[j]
            )
            return unitVect
          })
      },
      updateDeckPointsInForce() {
        ...
        this.deckPointsInForce = Array(this.nSegmentsInDeck + 1)
          .fill([0, 0])
          .map((elem, i) => [x, this.cablePointsInForce[i][1]])
      },
      updateTowerPointsInForce() {
        const towerSlope = this.towerVector[1] / this.towerVector[0]
        const x = this.cablePointsInForce[this.cablePointsInForce.length - 1][0]
        this.towerPointsInForce = Array(this.nSegmentsInDeck + 1)
          .fill([0, 0])
          .map((elem, i) => {
      ...
      update() {
        ...
        this.forceScaleNormalizer =
          this.deckVector[0] / (this.deckPointsInForce[0][1] - this.deckPointsInForce[1][1])
      },
      drawDeckPointsInForm() {
        ...
        this.deckPointsInForm.map(([x, y]) => {
          this.p5?.ellipse(
            x,
            y,
            this.ellipseSize / this.formScalar,
            this.ellipseSize / this.formScalar
          )
        })
      },
      ...
      drawTowerSegmentsInForm() {
        ...
        this.p5?.noFill()
        Array(this.nSegmentsInDeck)
          .fill(0)
          .map((elem, i) => {
            const [x1, y1] = this.towerPointsInForm[i].map(
              (coord, j) => coord - 0.5 * this.towerVector[j]
            )
            const [x2, y2] = this.towerPointsInForm[i].map(
              (coord, j) => coord + 0.5 * this.towerVector[j]
            )
      ...
      drawDeckSegmentsInForm() {
        ...
        Array(this.nSegmentsInDeck)
          .fill(0)
          .map((elem, i) => {
            const [x1, y1] = this.deckPointsInForm[i].map(
              (coord, j) => coord - 0.5 * this.deckVector[j]
            )
            const [x2, y2] = this.deckPointsInForm[i].map(
              (coord, j) => coord + 0.5 * this.deckVector[j]
            )
            this.p5?.line(x1, y1, x2, y2)
          })
      ...
      drawTicksInForm() {
        ...
        Array(this.nSegmentsInDeck)
          ...
          .map((elem, i) => {
            ...
            rotatedTowerVec
              ? this.p5?.line(
                  x2 + rotatedTowerVec.x,
                  y2 + rotatedTowerVec.y,
                  x2 - rotatedTowerVec.x,
                  y2 - rotatedTowerVec.y
                )
              : null
      ...
      drawCablePointsInForce() {
        ...
        this.cablePointsInForce.map(([x, y]) => {
          this.p5?.ellipse(
            x,
            y,
            this.ellipseSize / (this.forceScaleNormalizer * this.forceScalar),
            this.ellipseSize / (this.forceScaleNormalizer * this.forceScalar)
          )
        })
      },
      ...
      drawFormDiagram() {
        this.p5?.push()
        this.p5?.translate(
          this.p5?.width * this.formOffset[0],
          this.p5?.height * this.formOffset[1]
        )
      ...
      drawForceDiagram() {
        this.p5?.push()
        this.p5?.translate(
          this.p5?.width * this.forceOffset[0],
          this.p5?.height * this.forceOffset[1]
        )
  ...
  return (
    <>
      ...
      >
        <ToggleButtonGroup
          exclusive
          onChange={(event: React.MouseEvent<HTMLElement>, newInputGroup: string) =>
            setInputGroup(newInputGroup)
          }
          value={inputGroup}
        >
        ...
        <Typography variant="overline">
          GWP: x lbsCO2eq/transit, y years of driving to work, z flights from LA to NY, w steaks{' '}
        </Typography>
      </Stack>
      <Stack sx={{ mt: '2rem' }} direction="row" spacing="2rem" alignItems="center">
        <Box sx={{ width: '30%', display: inputGroup == 'geometric' ? 'inline' : 'none' }}>
          ...
          <Slider
            size="small"
            onChange={(e) => (calatrava.current.nSegmentsInDeck = e.target.value)}
            defaultValue={8}
            step={1}
            min={3}
            max={16}
          ...
          <Slider
            size="small"
            onChange={(e) => (calatrava.current.deckVector = [e.target.value, 0])}
            defaultValue={1}
            step={0.01}
            min={0.1}
            max={2.5}
      ...
      <Typography
        sx={{ display: inputGroup !== 'geometric' ? 'inline' : 'none', ml: '2rem' }}
        variant="overline"
      >
        Coming soon.
      </Typography>
